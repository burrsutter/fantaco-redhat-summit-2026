...
#
# Installs OpenShell into the current oc project namespace, registers
# the gateway, and creates the LLM provider.
#
# Port-forwarding is handled automatically by openshell.sh.
#
...
if [ -n "$PROVIDER_API_KEY" ]; then
  ...
  echo "Run later: ./openshell.sh provider create --name ${PROVIDER_NAME} --type generic --credential ${PROVIDER_API_KEY_VAR}"
  echo ""
fi
...
echo "============================================"
echo "  OpenShell ready in $NAMESPACE"
echo "============================================"
echo ""
